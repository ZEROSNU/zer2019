...
class tracker: # Fill it!  
        ...
        def S_path_update(self, temp_control_mode):
                try:
                        ...
                        for i in self.latest_path.poses:
                                self.cur_path.poses[self.latest_path.poses.index(i)] = i
                except:
        ...
        def S_path_estimate(self, cur_time, temp_control_mode):
                try:
                        ...
                        if past_control.steer != 0:
                                ...
                                for i in self.cur_path.poses:
                                        i.pose.position.y = i.pose.positon.y - delta_pos_y
                                        if (i.pose.position.x**2 + i.pose.position.y**2) < min_dis_square:
                                                min_point_seq = i.header.seq
                                        i.header.stamp = rospy.get_rostime().to_sec()
        ...
        def main_control_loop(self):
                ...
                try:
                        #self.upsign_path == True:    #New path is ready
                ...
                print(self.cur_path.poses[0].seq)
                ...
                if temp_control_mode == self.EMERGENCY_BRAKE:
                        ...
                        look_ahead_point = self.Set_look_ahead_point()
                        temp_control_mode =  self.Deicide_steering_angle(look_ahead_point, temp_control_mode)
