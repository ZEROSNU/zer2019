...
namespace ompl {
    ...
    base::PlannerStatus doRRT::solve(const base::PlannerTerminationCondition &ptc) {
        ...
        std::vector<int> nbh;
        while(ptc == false) {
            auto *rmotion = new Motion(siC_); // random sampled motion
            ...
            auto *nmotion = new Motion(siC_); // DOextended, or steered motion
            ...
            for(int i = 0 ; i<nbh.size() ; i++) {
                ...
                if(collisionFree(near, nmotion) && (cmin > near->cost + distanceMotion(near, nmotion))) {
                    minmotion = near;
        ...
        for(Motion* it = neargoal[mincostind] ; (interpolate_ ? it->parent!=nullptr : it!=nullptr) ; it = it->parent) {
            ...
            if(interpolate_) {
                auto *sp = new base::ReedsSheppStateSpace();
                double d = distanceMotion(it, it->parent);
                double deltat = res_/d;
                for(int i = 1; i * deltat < 1 ; i++) {
                    sp->interpolate(it->state, it->parent->state, i*deltat, temp);
                    path->append(temp);
                }
                delete sp;
            }
        ...
        if(interpolate_ && arrived) {
            ...
            double deltat = res_/d;
            for(int i = 1; i * deltat < 1 ; i++) {
                sp->interpolate(neargoal[mincostind]->state, goalmotion->state, i*deltat, temp);
                path->append(temp);
            }
            delete sp;
        }
    ...
    bool doRRT::DOExtend(Motion *nearest, Motion *rand, Motion *node) {
        base::State* goal = pdef_->getGoal().get()->as<base::GoalState>()->getState();
        auto *sp = new base::ReedsSheppStateSpace();
        double d1 = sp->distance(nearest->state, rand->state);
        double deltat1 = res_/d1;
        base::State* temp(si_->allocState());
        bool isvalid = true;
        for (int i = 0 ; (deltat1 * i < 1) && (i * res_ < extendlength_) ; i++) {
            sp->interpolate(nearest->state, rand->state, deltat1 * i, temp);
            if(!si_->isValid(temp)) {
                break;
            }
        ...
        for(int i = 0 ; i < maxsample_ ; i++ ) {
            ...
            if(isvalid) {
                double d2 = sp->distance(nearest->state, node->state);
                double delta2 = res_/d2;
                for(int j = 0 ; delta2 * j < 1; j++) {
                    sp->interpolate(nearest->state, node->state, j*delta2, temp);
                    if(!si_->isValid(temp)) {
                        isvalid = false;
                        break;
                    }
    ...
    bool doRRT::collisionFree(Motion *m1, Motion *m2) {
        auto *sp = new base::ReedsSheppStateSpace();
        double d = sp->distance(m1->state, m2->state);
        double deltat = res_/d;

        base::State* temp(si_->allocState());
        for (int i = 0 ; deltat * i < 1 ; i++) {
            sp->interpolate(m1->state, m2->state, deltat * i, temp);
            if(!si_->isValid(temp)) {
                si_->freeState(temp);
                delete sp;
                return false;
    ...
    double doRRT::distanceMotion(const Motion *a, const Motion *b) {
        ...
        */
        auto *sp = new base::ReedsSheppStateSpace();
        double d = sp->distance(a->state, b->state);
        delete sp;
        return d;
    }
